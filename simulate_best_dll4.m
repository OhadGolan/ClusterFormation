...
function []=simulate_best_dll4(i_best, t_final_best, i_best_dll4)
...
precent_of_white_cells_tissue=0.75;     %precent of white cells in tissue
 ...
 i=i_best;
        ...
        for run=1:60
            ...
            for t=0:dt:T_final - 5                      %time step over all the processess from 0 to the final time
                ...
                if start_of_cluster==0                   % if the cluster hasn't started yet, check if the cluster is to start at this time point
                    ...
                    if start_of_cluster_rand<(1/(T_final+5))  %if the random number for the start of the cluster if lower than the probability for the start of the cluster - start the cluster
            ...
            i=i_best_dll4;
             ...
             p_recruitment_colored=p_recruitment*precent_of_colored_cells_tissue;        %the probability
            p_recruitment_white=p_recruitment*precent_of_white_cells_tissue;            %the probability
            for t=T_final-5:dt:T_final                     %time step over all the processess from the final T to the extra time time (for igG or alpha-dll4 treatment time)
...
title('Simulation of clusters - dll4 treatment');
xlabel('Cluster');
